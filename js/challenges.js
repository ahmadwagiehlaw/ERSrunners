...
function renderCoachHeroStats() {
    ...
    // 3.5) Fix: لو الكاش فاضي (أو لم يُحمّل بعد) نجلب آخر الجريات مرة واحدة فقط
    // (قراءة خفيفة: limit(50) مرة واحدة في الجلسة، لتفادي ضغط Firebase)
    try {
        const shouldHydrate = (!runs || runs.length === 0) && currentUser && db && !window.__ersHeroRunsHydrated;
        if (shouldHydrate) {
            window.__ersHeroRunsHydrated = true;
            if (!window.__ersHeroRunsHydratePromise) {
                window.__ersHeroRunsHydratePromise = db
            ...
            window.__ersHeroRunsHydratePromise.then(snap => {
                const fetched = [];
                snap.forEach(d => fetched.push(d.data()));
                // حافظ على الكاش لو كان فاضي
                if (!window._ersRunsCache || window._ersRunsCache.length === 0) {
                    window._ersRunsCache = fetched;
                }
                // حدّث الكارت باستخدام الدالة الآمنة إن وجدت
                if (typeof _ersUpdateCoachHeroStatsFromRuns === 'function') {
                    _ersUpdateCoachHeroStatsFromRuns(fetched);
                } else {
                    // fallback: إعادة استدعاء الدالة نفسها بعد تعبئة الكاش
