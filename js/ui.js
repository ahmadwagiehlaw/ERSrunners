...
function switchView(viewId) {
    ...
    if (viewId === 'club') {
        try { if (typeof loadGlobalFeedInitial === 'function') loadGlobalFeedInitial(); } catch(e) {}
    }
...
async function submitRun() {
    ...
    // GPS optional summary (local-only until submit)
    const gpsSummary = (window._ersLastGpsSummary && window._ersLastGpsSummary.pending) ? window._ersLastGpsSummary : null;
  ...
  return showToast("لازم ترفق صورة إثبات لتمرين الكوتش 📸", "error");
    ...
    try {
        ...
        if (editingRunId) {
            ...
            // Optional: attach GPS meta safely (does not affect any counters)
            if (gpsSummary) {
                const gpsEdited = (Math.abs(((isCore ? dist : 0) - (gpsSummary.distKm || 0))) > 0.05) || (Math.abs((time - (gpsSummary.timeMin || 0))) > 0.5);
                updatePayload.source = 'gps';
                updatePayload.stepsEst = (gpsSummary.stepsEst != null) ? gpsSummary.stepsEst : null;
                updatePayload.gps = {
                    start: gpsSummary.startMs ? firebase.firestore.Timestamp.fromDate(new Date(gpsSummary.startMs)) : null,
                    end: gpsSummary.endMs ? firebase.firestore.Timestamp.fromDate(new Date(gpsSummary.endMs)) : null,
                    points: gpsSummary.points || 0,
                    avgAccM: gpsSummary.avgAccM ?? null,
                    distKm: gpsSummary.distKm ?? null,
                    timeMin: gpsSummary.timeMin ?? null,
                    pace: gpsSummary.pace ?? null,
                    edited: gpsEdited
                };
            }
            ...
            if (gpsSummary) {
              const gpsEdited = (Math.abs(((isCore ? dist : 0) - (gpsSummary.distKm || 0))) > 0.05) || (Math.abs((time - (gpsSummary.timeMin || 0))) > 0.5);
              runData.source = 'gps';
              runData.stepsEst = (gpsSummary.stepsEst != null) ? gpsSummary.stepsEst : null;
              runData.gps = {
                start: gpsSummary.startMs ? firebase.firestore.Timestamp.fromDate(new Date(gpsSummary.startMs)) : null,
                end: gpsSummary.endMs ? firebase.firestore.Timestamp.fromDate(new Date(gpsSummary.endMs)) : null,
                points: gpsSummary.points || 0,
        ...
        // Clear GPS summary after successful save (so it won't attach to next run)
        try {
          if (window._ersLastGpsSummary && window._ersLastGpsSummary.pending) window._ersLastGpsSummary = null;
          if (typeof updateGpsLogPanel === 'function') updateGpsLogPanel();
        } catch(e) {}
