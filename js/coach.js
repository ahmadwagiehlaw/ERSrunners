...
function updateCoachDecisionUI(runsOverride){
    ...
    // Optional smart nudge: detect pace drop and alert once per day (safe, local only)
    try { _ersMaybeNotifyPaceDrop(runs); } catch(e) {}
}

function _ersMaybeNotifyPaceDrop(runs){
    if(!Array.isArray(runs) || runs.length < 3) return;
    const dateKey = (typeof _ersDateKey === 'function') ? _ersDateKey(new Date()) : null;
    const onceKey = dateKey ? `ers_pace_drop_notified_${dateKey}` : 'ers_pace_drop_notified';
    if(typeof localStorage !== 'undefined' && localStorage.getItem(onceKey) === '1') return;

    // pick last 3 valid run-like sessions (>=1km) with time
    const valid = runs
        .filter(r=>{
            ...
            return isRun && dist >= 1 && time > 0;
        })
        .slice(0, 6);
    if(valid.length < 3) return;

    const pace = (r)=> (parseFloat(r.time||0) / Math.max(0.001, parseFloat(r.dist||0))); // min/km
    const last = valid[0];
    const pLast = pace(last);
    const base = valid.slice(1,3).map(pace);
    const pBase = base.reduce((a,b)=>a+b,0)/base.length;
    if(!isFinite(pLast) || !isFinite(pBase) || pBase <= 0) return;

    // require meaningful drop: +12% and +0.6 min/km
    const worsePct = (pLast / pBase) - 1;
    const worseAbs = pLast - pBase;
    if(worsePct < 0.12 || worseAbs < 0.6) return;

    // only if recent (last run within 10 days)
    const toDateSafe = (x)=>{
        try { return x?.timestamp?.toDate ? x.timestamp.toDate() : (x?.date ? new Date(x.date) : null); } catch(e) { return null; }
    };
    const dLast = toDateSafe(last);
    if(!dLast) return;
    const days = (Date.now() - dLast.getTime()) / 86400000;
    if(days > 10) return;

    const msg = 'لاحظت إن البيس نزل في آخر جرية… جرّب تعمل يومين Easy/راحة واستعيد نشاطك 💡';
    try{ showToast(msg, 'info'); }catch(e){}

    // store once per day
    try{ if(typeof localStorage !== 'undefined') localStorage.setItem(onceKey, '1'); }catch(e){}
}
//========================================================
// دوال مساعدة للعرض
