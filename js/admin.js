...
async function adminDeleteWorkout(id) {
    if (!id) return;
    showConfirm('حذف التمرين؟ (سيختفي من مكتبة الجميع)', async () => {
        try {
...
async function toggleBan(uid, banStatus) {
    showConfirm(banStatus ? "حظر العضو؟" : "فك الحظر؟", async () => {
        try {
...
async function adminDelete(id, dist) {
    showConfirm("حذف هذا النشاط المشبوه؟", () => {
...
async function adminForceDelete(feedId, userId, runDist) {
    showConfirm("هل أنت متأكد من حذف هذا النشاط للمستخدم؟", async () => {
        // تغيير نص الزر ليعرف الأدمن أن العملية جارية
        const btn = (typeof event !== 'undefined' && event.target) ? event.target : document.createElement('button'); // fallback safely
        btn.innerText = "...";
        ...
        try {
            ...
            if (!feedDoc.exists) {
                ...
                if (btn.closest && btn.closest('.alert-card')) btn.closest('.alert-card').remove();
                return;
            }
...
// دالة أرشفة الشهر السابق
async function archiveLastMonth() {
    if (!confirm('هل تريد أرشفة بيانات الشهر الحالي؟\n\nسيتم نقل monthDist → lastMonthDist لجميع المستخدمين')) {
        return;
    }

    const btn = event.target;
    btn.innerText = 'جاري الأرشفة...';
    btn.disabled = true;
    ...
    try {
        ...
        for (const userDoc of usersSnap.docs) {
            const userData = userDoc.data();
            const monthDist = parseFloat(userData.monthDist) || 0;

            if (monthDist > 0) {
                await db.collection('users').doc(userDoc.id).update({
                    lastMonthDist: monthDist,
                    monthDist: 0 // تصفير للشهر الجديد
                });
                archived++;
            }
        }

        showToast(`✅ تم أرشفة ${archived} عضو`, 'success');
        setTimeout(loadLeagueDiagnostics, 1000);
