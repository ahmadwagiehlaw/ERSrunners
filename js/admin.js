...
// ============= تطوير ادوات الأدمن (V3.0+) =============
async function loadObserverRuns() {
  const list = document.getElementById('observer-runs-list');
  if (!list) return;

  list.innerHTML = '<p style="text-align:center;color:#9ca3af;">جاري تحميل الجريات...</p>';
  ...
  if (snap.empty) {
    list.innerHTML = '<p style="text-align:center;">لا توجد جريات</p>';
    return;
  }
  ...
  for (const doc of snap.docs) {
    ...
    const pace = dist > 0 && time > 0 ? (time / dist).toFixed(1) : '-';

    html += `
      <div class="log-row-compact admin-observer" data-run-id="${doc.id}">
        <div class="log-details">
          <div class="log-main-stat">
            ${user.name || 'مستخدم غير معروف'}

            <div class="run-details-box" id="run-details-${doc.id}" style="display:none;"></div>
...
<button class="btn-icon-action"
  onclick="toggleRunDetails('${userId}', '${doc.id}')">
...
async function toggleRunDetails(userId, runId) {
  const box = document.getElementById(`run-details-${runId}`);
  if (!box) return;
  ...
  box.innerHTML = '<div style="padding:10px;color:#9ca3af;">جاري تحميل التفاصيل...</div>';

  try {
    const runRef = db.collection('users').doc(userId).collection('runs').doc(runId);
    const snap = await runRef.get();
    if (!snap.exists) {
      box.innerHTML = '<div style="color:#ef4444;">الجرية غير موجودة</div>';
      return;
    }
    ...
    const dist = Number(r.dist) || 0;
    const time = Number(r.time) || 0;
    const pace = dist > 0 && time > 0 ? (time / dist).toFixed(2) : '-';

    box.innerHTML = `
      <div class="run-details-inner">
        <div><strong>📏 المسافة:</strong> ${dist} كم</div>
        <div><strong>⏱️ الزمن:</strong> ${time} دقيقة</div>
        <div><strong>⚡ السرعة:</strong> ${pace} د/كم</div>
        <div><strong>🏷️ النوع:</strong> ${r.type || '-'}</div>
        <div><strong>🗓️ التاريخ:</strong> ${
          r.timestamp ? r.timestamp.toDate().toLocaleString('ar-EG') : '-'
        }</div>
        <div><strong>📍 المصدر:</strong> ${r.source || 'manual'}</div>

        ${r.suspicious ? `<div class="warn">⚠️ جرية مشبوهة</div>` : ''}
        ${r.excluded ? `<div class="warn">🚫 مستبعدة من الإحصائيات</div>` : ''}
        ${r.adminFix ? `<div class="info">🛠️ تم تعديلها إداريًا</div>` : ''}
      </div>
    `;
  } catch (e) {
    box.innerHTML = '<div style="color:#ef4444;">حدث خطأ أثناء التحميل</div>';
  }
}
